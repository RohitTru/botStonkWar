...
class SentimentDivergenceStrategy(BaseStrategy):
    ...
    def __init__(self, confidence_threshold=0.8, lookback=5, divergence_threshold=0.3):
        super().__init__(
            name="sentiment_divergence",
            description="Identifies stocks where sentiment and price action diverge significantly."
        )
    ...
    def fetch_live_price(self, symbol):
        ...
        try:
            ...
            print(f"[Divergence] Could not fetch live price for {symbol}: {e}")
            return {
                'price': None,
                'change_percent': None,
                'volume': None,
    ...
    def analyze(self, data):
        ...
        try:
            ...
            error = str(e)
            print(f"[Divergence] Error analyzing data: {e}")
